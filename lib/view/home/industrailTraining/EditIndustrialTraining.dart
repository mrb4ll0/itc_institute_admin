...
class EditIndustrialTrainingPage extends StatefulWidget {
  ...
  @override
  State<EditIndustrialTrainingPage> createState() =>
      _EditIndustrialTrainingPageState();
}

class _EditIndustrialTrainingPageState
    extends State<EditIndustrialTrainingPage> {
  ...
  List<String> _existingFileUrls = []; // Store existing file URLs
  ...
  void _loadTrainingData() {
    ...
    _existingFileUrls =
        (training.files as List<dynamic>?)
            ?.map((file) => file.toString())
            .toList() ??
        [];

    // Set form values from training data
    _titleController.text = training.title;
    _departmentController.text = training.department ?? '';
    _locationController.text = training.location;
    _descriptionController.text = training.description;
    _skillsController.text = training.eligibilityCriteria ?? '';
    _intakeController.text = training.intake?.toString() ?? '0';
    _stipendController.text = training.stipend ?? '';
    _contactPersonController.text = training.contactPerson ?? '';

    // Set dropdown values
    _aptitudeTest = training.aptitudeTestRequired ? 'Yes' : 'No';
    _status = training.status?.isNotEmpty == true
        ? training.status![0].toUpperCase() + training.status!.substring(1)
        : 'Open';

    _formUsage = training.isUniversalForm ? 'universal' : 'single';
  }
  ...
  Future<void> _updateTraining() async {
    ...
    try {
      ...
      if (_selectedFiles.isNotEmpty) {
        newAttachmentUrls = await _uploadFiles();
      }

      // Combine existing and new file URLs
      final allAttachmentUrls = [..._existingFileUrls, ...newAttachmentUrls];
      ...
      final updatedTraining = IndustrialTraining(
        id: widget.training.id, // Preserve the ID
        company: company,
        title: _titleController.text.trim(),
        industry: company.industry,
        duration: widget.training.duration, // Preserve existing
        startDate: widget.training.startDate, // Preserve existing
        endDate: widget.training.endDate, // Preserve existing
        department: _departmentController.text.trim(),
        location: _locationController.text.trim(),
        description: _descriptionController.text.trim(),
        applicationsCount: widget.training.applicationsCount, // Preserve count
        intake: int.tryParse(_intakeController.text.trim()) ?? 0,
        status: _status!.toLowerCase(),
        stipendAvailable: _stipendController.text.trim().isNotEmpty,
        stipend: _stipendController.text.trim(),
        eligibilityCriteria: _skillsController.text.trim(),
        files: allAttachmentUrls,
        aptitudeTestRequired: _aptitudeTest == 'Yes',
        isTemplate: _formUsage == 'universal',
        isUniversalForm: _formUsage == 'universal',
        contactPerson: _contactPersonController.text.trim(),
        createdAt: widget.training.createdAt, // Preserve original creation date
        postedAt: widget.training.postedAt, // Preserve original post date
        updatedAt: DateTime.now(), // Update with current time
      );

      debugPrint(
        "_contactPersonController.text.trim() ${_contactPersonController.text.trim()}",
      );
      ...
      if (_formUsage == 'universal') {
        ...
        // Update template in training_templates collection
        await FirebaseFirestore.instance
            ...
            .set({
              'template': {
                ...
                'requiredSkills': updatedTraining.eligibilityCriteria,
                'aptitudeTestRequired': updatedTraining.aptitudeTestRequired,
                'intake': updatedTraining.intake,
                'stipend': updatedTraining.stipend,
                'contactPerson': updatedTraining.contactPerson,
                'attachmentUrls': updatedTraining.files,
              },
      ...
      _showError('Failed to update training opportunity: $e');
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
  ...
  Widget _buildFilePreview() {
    ...
    return Column(
      ...
      children: [
        ...
        // Show existing files
        ..._existingFileUrls.asMap().entries.map((entry) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Edit Industrial Training'),
        backgroundColor: isDark ? const Color(0xFF1a2036) : Colors.white,
        elevation: 0,
        leading: IconButton(
                    ...
                    children: [
                      ...
                      Center(
                        child: Column(
                          children: [
                            Text(
                              'Edit Industrial Training Opportunity',
                            ...
                            const SizedBox(height: 8),
                            Text(
                              'Update the details below to modify this industrial training opportunity.',
                            ...
                            Column(
                              children: [
                                ...
                                Column(
                                  ...
                                  children: [
                                    Text(
                                      'Attach Files (Add new or remove existing)',
  ...
  Future<void> _showDeleteConfirmation() async {
    return showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text(
          'Are you sure you want to delete this training opportunity? This action cannot be undone.',
        ),
        actions: [
          TextButton(
  ...
  Future<void> _deleteTraining() async {
    ...
    try {
      ...
      await FirebaseFirestore.instance
          .collection('industrial_trainings')
          .doc(widget.training.id)
          .delete();

      // Also delete from company_cloud if needed
      await company_cloud.deleteInternship(widget.training);

      _showSuccess('Training opportunity deleted successfully!');
      ...
      _showError('Failed to delete training: $e');
    } finally {
      if (mounted) {
        setState(() {
          _isLoading = false;
